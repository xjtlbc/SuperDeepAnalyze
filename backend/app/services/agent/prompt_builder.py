...
<section id="answer_format">
## 答案输出格式

回答请按以下Markdown结构组织，使用`## `标题分隔各节：
...
## 参考来源
（列出所有引用的文档ID，格式: - doc_xxx (文件名)）
```

如果某节不适用，标注"（不适用）"并跳过。不是每个答案都需要所有节。
标注引用时使用格式：[doc_xxx L15] 或 [doc_xxx]。
</section>
